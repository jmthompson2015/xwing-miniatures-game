...
define(["qunit", "redux",
  "artifact/js/DiceModification", "artifact/js/Difficulty", "artifact/js/Faction", "artifact/js/Maneuver", "artifact/js/PilotCard",
  "model/js/Action", "model/js/Adjudicator", "model/js/CombatAction", "model/js/Environment", "model/js/EnvironmentAction", "model/js/MediumAgent", "model/js/Position", "model/js/Reducer", "model/js/Squad", "model/js/SquadBuilder", "model/js/TargetLock", "model/js/Token", "model/js/TokenAction",
  "../../../test/model/js/EnvironmentFactory", "../../../test/model/js/MockAttackDice", "../../../test/model/js/MockDefenseDice"],
   function(QUnit, Redux, DiceModification, Difficulty, Faction, Maneuver, PilotCard,
      Action, Adjudicator, CombatAction, Environment, EnvironmentAction, MediumAgent, Position, Reducer, Squad, SquadBuilder, TargetLock, Token, TokenAction,
      EnvironmentFactory, MockAttackDice, MockDefenseDice)
   {
      ...
      {
         // Setup.
         var result = new MediumAgent("myAgent", "myFaction");
      ...
      QUnit.test("chooseWeaponAndDefender() Imperial", function(assert)
      {
         ...
         var agent = new MediumAgent(name, faction);
      ...
      QUnit.test("cloneStore()", function(assert)
      {
         ...
         assert.equal(attacker.pilotKey(), PilotCard.POE_DAMERON);
         assert.equal(defender.pilotKey(), PilotCard.EPSILON_LEADER);
      ...
      QUnit.test("getModifyAttackDiceAction() pilot", function(assert)
      {
         ...
         function callback(modifyAbility)
         {
            ...
            // Verify.
            assert.ok(modifyAbility);
            assert.equal(modifyAbility.sourceKey(), PilotCard.POE_DAMERON);
         }
      ...
      QUnit.test("getModifyDefenseDiceAction() pilot", function(assert)
      {
         ...
         function callback(modifyAbility)
         {
            ...
            // Verify.
            assert.ok(modifyAbility);
            assert.equal(modifyAbility.sourceKey(), PilotCard.LUKE_SKYWALKER);
         }
      ...
      QUnit.test("getPlanningAction() IG-88A", function(assert)
      {
         ...
         var firstAgent = EnvironmentFactory.createAgent(MediumAgent, "First Agent", Faction.IMPERIAL, iconBase, imageBase);
         var firstTokens = [new Token(store00, PilotCard.MAULER_MITHEL, firstAgent)];
         var firstSquad = new Squad(Faction.IMPERIAL, "First Squad", 2017, "description", firstTokens);
         var secondAgent = EnvironmentFactory.createAgent(MediumAgent, "Second Agent", Faction.SCUM, iconBase, imageBase);
         var secondTokens = [new Token(store00, PilotCard.IG_88A, secondAgent)];
      ...
      {
         // Setup.
         var agent = new MediumAgent("myAgent", Faction.IMPERIAL);
