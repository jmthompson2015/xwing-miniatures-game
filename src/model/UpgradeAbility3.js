...
define(["utility/InputValidator",
  "artifact/AttackDiceValue", "artifact/DefenseDiceValue", "artifact/Faction", "artifact/Phase", "artifact/Range", "artifact/ShipAction", "artifact/UpgradeCard", "artifact/UpgradeType",
  "model/Ability", "model/Action", "model/AttackDice", "model/CardAction", "model/CombatAction", "model/DefenseDice", "model/RangeRuler", "model/Selector", "model/ShipActionAbility", "model/TargetLock"],
      ...
      Ability, Action, AttackDice, CardAction, CombatAction, DefenseDice, RangeRuler, Selector, ShipActionAbility, TargetLock)
   {
      ...
      UpgradeAbility3[Phase.COMBAT_MODIFY_ATTACK_DICE][UpgradeCard.GUIDANCE_CHIPS] = {
         ...
         consequent: function(store, token, callback)
         {
            ...
            var oldValue = (attackDice.blankCount() > 0 ? AttackDiceValue.BLANK : AttackDiceValue.FOCUS);
            var newValue = (weapon.weaponValue() >= 3 ? AttackDiceValue.CRITICAL_HIT : AttackDiceValue.HIT);
      ...
      UpgradeAbility3[Phase.COMBAT_MODIFY_DEFENSE_DICE][UpgradeCard.AUTOTHRUSTERS] = {
         ...
         condition: function(store, token)
         {
            ...
            var isBeyondRange2 = ![Range.ONE, Range.TWO].includes(rangeKey);
      ...
      UpgradeAbility3[Phase.COMBAT_MODIFY_DEFENSE_DICE][UpgradeCard.JESS_PAVA] = {
         ...
         condition: function(store, token)
         {
            ...
            var defenseDice = getDefenseDice(attacker);
            var environment = getEnvironment(store);
            var friendlyCount = environment.getFriendlyTokensAtRange(defender, Range.ONE).length;
